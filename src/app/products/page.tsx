...
export default function PageProducts() {
  ...
  return (
    <main className="w-full bg-background-homepage flex flex-col items-center px-4 pb-[25px]">
        ...
        {products.map((prod) => (
          <div key={prod.id} className="text-center group relative">
            ...
            <div className="hidden lg:group-hover:flex flex-col gap-2 items-center justify-center absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black/81 p-4 rounded-[170px] w-[235px] xl:w-[292px] lg:h-[396px] xl:h-[486px] shadow-lg">
              <h2 className="font-['josefin_Sans'] text-[24px] text-default-text" >{t(prod.nameKey)}</h2>
              <p className="font-['julius_Sans_One'] text-[20px] text-default-text"> {t('products.coin')} {prod.price}</p>
              <button
                onClick={() => router.push(`/products/${prod.id}`)}
            ...
            <div className="lg:hidden mt-2">
              <h2 className="font-['josefin_Slab'] text-[18px] font-semibold text-default-text">{t(prod.nameKey)}</h2>
              <p className="font-['julius_Sans_One'] text-[14px] text-default-text">{t('products.coin')} {prod.price}</p>
              <button
                onClick={() => handleAddToCart(prod.id)}
                className="border border-[#677146] text-default-text font-['julius_Sans_One'] text-[16px]  rounded-[20px] mt-[10px] pl-[3px] pr-[3px] hover:bg-button-pages hover:text-default-text cursor-pointer"
              >
